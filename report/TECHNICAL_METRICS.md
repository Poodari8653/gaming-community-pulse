...
- **Scope, by product decision**: whichever of YouTube, Reddit, Discord and Twitch are actually configured and returned live rows this refresh, combined per game — never the illustrative sample. `server.js` passes `lib/gemini.js` the `enriched` array from step 2 (§2) directly — every live record across every platform, already scored — rather than raw collector output; the module itself has no platform-specific logic beyond tagging each record with its `platform` for attribution, so a fifth platform wrapper would be included automatically with no change needed here. Clustering sample text and presenting it as real community discussion would be a materially worse failure than the panel simply saying a game doesn't have enough live volume yet.
- **Grounding is the core design constraint, and it now covers sentiment too.** Gemini is shown a numbered list of records — `{ id, platform, text (≤300 chars), sentiment (label, already computed), engagement (0-100 Engagement Index, already computed) }`, explicitly *without* the URL — and asked only to (a) name 3–5 sub-topics present in the data and (b) list which record ids best represent each one (`quote_ids`, 2–4 per cluster). The model is never asked to produce quote text or state a sentiment/engagement figure, and the system prompt states this directly: *"Never write out the quote text yourself, and never state a sentiment or engagement figure yourself — you are selecting existing records, not quoting or scoring from memory."* The application then looks up the real text, permalink, platform, `sentiment_score`, `sentiment_label`, `engagement_index` and `is_risk` for each returned id from its own already-enriched data. An id the model returns that doesn't match a real record is silently dropped, never fabricated into a placeholder quote; a cluster left with zero resolvable quotes is dropped entirely rather than shown with no evidence behind it. A cluster's `avg_sentiment` and `has_risk` fields are then computed with plain arithmetic over those resolved quotes — never asked of the model — so the same grounding guarantee that protects quote text and links extends to every number the panel shows. This means the panel's links (and now its sentiment/risk figures) are exactly as trustworthy as the rest of the dashboard's — all read from enriched data, never model output — even though the *clustering judgement* (which sub-topics exist, which records best represent them) is entirely the model's. Each returned summary also carries a `platforms` array and an `avg_engagement_of_pool` figure describing the pool sent for that game.
- **Ranked by Engagement Index, not each platform's own metric.** The up-to-60 records sent per game are the highest-`engagement_index` ones for that game this refresh, mirroring what a human moderator skimming "sort by top" would actually see — not a random or chronological sample, and not each platform's raw native metric (Reddit upvotes, YouTube likes, Discord reaction count, Twitch view count) separately, which aren't comparable to each other. This closes a limitation of the first version of this feature, which ranked on raw per-platform scores.
- **Twitch is a special case worth flagging.** A Twitch record's text is the clip's title, written by the streamer or clipper as promotion, not a player's own words. The system prompt tells the model this directly and instructs it not to let clip titles dominate a cluster meant to represent community sentiment.
- **Minimum volume**: fewer than 6 combined live records for a game (across every platform contributing to the pool) and clustering is skipped for that game (`available: false`, with a specific reason), on the reasoning that clustering 3-4 records into "3-5 sub-topics" produces noise dressed as insight, not a genuine finding.
...
- **Panels**: Today's briefing → KPI row → Discussion volume & sentiment over time → Sentiment mix by game → Engagement Index by game → How the channels differ (platform breakdown) → Recurring themes → Publication region → Recurring questions → Highest-impact posts → Twitch live-right-now → Top discussions (Gemini-clustered, all live platforms) → Browse the underlying records (raw table) → live "How is this calculated?" methodology panel.
...
- Cross-platform discussion clustering only ever describes real, live rows — never the illustrative sample — so a game with too little combined live volume this refresh (or no platform configured at all) reports itself unavailable rather than clustering placeholder text and presenting it as real discussion. Its ranking metric for "which records to feature" is each platform's own native score, not the Engagement Index, so it isn't a cross-platform-comparable figure.
